...
public class MT863DeviceSupport extends AbstractBTLEDeviceSupport {
    ...
    private void handleGetAlarmsResponse(byte[] payload)
    {
        ...
        for (nodomain.freeyourgadget.gadgetbridge.entities.Alarm alarm : alarms) {
            ...
            alarm.setRepetition(AlarmUtils.createRepetitionMassk(
                (repetition & 2) != 0,
                (repetition & 4) != 0,
                (repetition & 8) != 0,
                (repetition & 16) != 0,
        ...
        for(int i = 0; i < data.length / 24; i++)
        {
            ...
            try (DBHandler dbHandler = GBApplication.acquireDB()) {
                ...
                else
                {
                    ...
                    String name;
                    if (type == MT863SampleProvider.ACTIVITY_TRAINING_ROPE)
                        name = "Rope";
                    else if (type == MT863SampleProvider.ACTIVITY_TRAINING_BADMINTON)
                        name = "Badminton";
                    else if (type == MT863SampleProvider.ACTIVITY_TRAINING_BASKETBALL)
                        name = "Basketball";
                    else if (type == MT863SampleProvider.ACTIVITY_TRAINING_FOOTBALL)
                        name = "Football";
                    else if (type == MT863SampleProvider.ACTIVITY_TRAINING_MOUNTAINEERING)
                        name = "Mountaineering";
                    else if (type == MT863SampleProvider.ACTIVITY_TRAINING_TENNIS)
                        name = "Tennis";
                    else if (type == MT863SampleProvider.ACTIVITY_TRAINING_RUGBY)
                        name = "Rugby";
                    else if (type == MT863SampleProvider.ACTIVITY_TRAINING_GOLF)
                        name = "Golf";
                    else
